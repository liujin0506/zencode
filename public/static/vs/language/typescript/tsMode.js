...
define("vs/language/typescript/tsMode", ["require", "require"], require => {
  var moduleExports = (() => {
    ...
    var ne = (s, e) => () => (
      ...
      ae = (s => (e, t) =>
        (s && s.get(e)) || ((t = D($({}), e, 1)), s && s.set(e, t), t))(
            ...
            (e = e.then(t =>
              this._worker
                ? this._worker.withSyncedResources(
                    i.editor
                      .getModels()
                      .filter(r => r.getLanguageId() === this._modeId)
                      .map(r => r.uri)
                  )
                : t
            )),
    ...
    var y = class {
      ...
      L = class {
        ...
        getOrCreateModel(e) {
          ...
          return o ? i.editor.createModel(o.content, "typescript", t) : null;
        }
        _containsLibFile(e) {
    ...
    var A = class extends y {
        constructor(e, t, r, o) {
          ...
          let u = n => {
              ...
              let c = () => {
                ...
                m = n.onDidChangeContent(() => {
                  clearTimeout(p), (p = window.setTimeout(c, 500));
                }),
          ...
          this._disposables.push(i.editor.onDidCreateModel(n => u(n))),
        ...
        async _doValidate(e) {
          ...
          let n = g
            ...
            c = n
              ...
              .reduce((p, m) => m.concat(p), [])
              .map(p => (p.file ? i.Uri.parse(p.file.fileName) : null));
        ...
        _convertRelatedInformation(e, t) {
          ...
          return (
            t.forEach(o => {
              ...
              if (
                (o.file &&
                  (u = this._libFiles.getOrCreateModel(o.file.fileName)),
                !u)
              )
                return;
              let d = o.start || 0,
                g = o.length || 1,
                { lineNumber: n, column: c } = u.getPositionAt(d),
                { lineNumber: p, column: m } = u.getPositionAt(d + g);
              r.push({
                resource: u.uri,
                startLineNumber: n,
                startColumn: c,
      ...
      x = class extends y {
            ...
            : {
                suggestions: p.entries.map(b => {
                  ...
                  if (b.replacementSpan) {
                    let T = e.getPositionAt(b.replacementSpan.start),
                      P = e.getPositionAt(
                        b.replacementSpan.start + b.replacementSpan.length
                      );
                    h = new i.Range(
                      T.lineNumber,
                      T.column,
                  ...
                  let _ = [];
                  return (
                    b.kindModifiers?.indexOf("deprecated") !== -1 &&
                      _.push(i.languages.CompletionItemTag.Deprecated),
                    {
                      uri: g,
                      position: t,
                      offset: n,
        ...
        async resolveCompletionItem(e, t) {
          let r = e,
            ...
            u = r.position,
            d = r.offset,
            n = await (await this._worker(o)).getCompletionEntryDetails(
              o.toString(),
              d,
              r.label
            );
          return n
            ? {
                ...
                documentation: { value: x.createDocumentationString(n) }
              }
            : r;
        }
...
${X(r)}`;
    ...
    function X(s) {
      ...
      if (s.name === "param" && s.text) {
        let [t, ...r] = s.text;
        (e += `\`${t.text}\``),
          r.length > 0 && (e += ` \u2014 ${r.map(o => o.text).join(" ")}`);
      } else
              ...
              (m.documentation = { value: S(p.documentation) }),
                ...
                p.parameters.forEach((b, h, _) => {
                  let T = S(b.displayParts),
                    P = {
                      label: T,
                      documentation: { value: S(b.documentation) }
                    };
                  (m.label += T),
                    m.parameters.push(P),
                    h < _.length - 1 && (m.label += S(p.separatorDisplayParts));
                }),
        ...
        async provideDocumentSymbols(e, t) {
          ...
          let d = (n, c, p) => {
              let m = {
                name: c.text,
                detail: "",
                kind: k[c.kind] || i.languages.SymbolKind.Variable,
                range: this._textSpanToRange(e, c.spans[0]),
                selectionRange: this._textSpanToRange(e, c.spans[0]),
                tags: []
              };
              if (
                (p && (m.containerName = p),
...
`
      ...
      U = class extends v {
        async provideCodeActions(e, t, r, o) {
          ...
          if (e.isDisposed()) return;
          let m = await p.getCodeFixesAtPosition(u.toString(), d, g, c, n);
          return !m || e.isDisposed()
            ? { actions: [], dispose: () => {} }
            : {
                actions: m
                  .filter(h => h.changes.filter(_ => _.isNewFile).length === 0)
                  .map(h => this._tsCodeFixActionToMonacoCodeAction(e, r, h)),
                dispose: () => {}
              };
        }
      ...
      V = class extends y {
        ...
        async provideRenameEdits(e, t, r, o) {
          let u = e.uri,
            ...
            n = await this._worker(u);
          if (e.isDisposed()) return;
          let c = await n.getRenameInfo(d, g, { allowRenameOfImportPath: !1 });
          if (c.canRename === !1)
            return { edits: [], rejectReason: c.localizedErrorMessage };
          ...
          for (let b of p) {
            ...
            if (h)
              m.push({
                resource: h.uri,
                edit: { range: this._textSpanToRange(h, b.textSpan), text: r }
              });
      ...
      K = class extends y {
        async provideInlayHints(e, t, r) {
          ...
          return e.isDisposed()
            ? null
            : {
                hints: (await n.provideInlayHints(u, d, g)).map(m => ({
                  ...m,
                  label: m.text,
    ...
    function Y(s, e) {
      ...
      return (
        ...
        i.languages.registerDocumentRangeFormattingEditProvider(e, new W(r)),
