...
const MESSAGES_MAP = {
  ...
  col: rowIdx => `第 ${rowIdx} 行, 期望是一个冒号！`,
  end: (rowIdx, brc) => `第 ${rowIdx} 行, 期望是一个逗号或 \"${brc}\"`
};
  ...
  return new Promise(function(resolve) {
    ...
    try {
      try {
        ...
        if (
          fmtSource === "" ||
          ["object", "boolean"].indexOf(typeof fmtSource) > -1
        ) {
    ...
    function doSpecialFormat() {
      ...
      if (!isMatched) {
        const numbMt = fmtSource.match(
          /^(-?[0-9]+\.?[0-9]*|0[xX][0-9a-fA-F]+)/
        );
        if (numbMt) {
          isMatched = true;
